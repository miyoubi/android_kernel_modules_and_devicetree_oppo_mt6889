...
static void pen_checkdown_work(struct work_struct *work)
{
...
static void init_goodix_chip_dts(struct device *dev, void *chip_data)
{
	...
	if (!chip_np) {
		TPD_INFO("GT:%s: fail get child node for gt9916", __func__);
		return;
	} else {
		TPD_INFO("GT:%s: success get child node for gt9916", __func__);
	}
	...
	if (rc) {
		TPD_INFO("GT:%s: fail get gt9916_get_motor_coord %d\n", __func__, rc);
	} else {
	...
	if (rc) {
		TPD_INFO("GT:%s: fail get gt9916_dynamic_limit %d\n", __func__, rc);
	} else {
