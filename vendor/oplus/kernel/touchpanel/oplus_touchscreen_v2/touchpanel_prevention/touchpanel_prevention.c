...
static bool edge_sliding_exit_matched(struct kernel_grip_info *grip_info, struct point_info *points, int index)
{
	...
	if (cur_p.rx_er * cur_p.rx_press >= grip_info->edge_sliding_exit_yfsr_thd) {
		if (abs(cur_p.y - first_p.y) <= grip_info->edge_sliding_exit_distance) {
...
static const struct key_addr key_addr_arrays[] = {
	...
	{"edge_sliding_exit_yfsr_thd", 	(u64)&(((struct kernel_grip_info *)0)->edge_sliding_exit_yfsr_thd)},
	{"edge_sliding_exit_distance", 	(u64)&(((struct kernel_grip_info *)0)->edge_sliding_exit_distance)},
};
...
static int kernel_grip_init_v4(struct kernel_grip_info *grip_info, struct device *dev)
{
	...
	if (ret) {
		...
		grip_info->edge_sliding_exit_yfsr_thd = 0xffff;
		grip_info->edge_sliding_exit_distance = grip_info->max_y / 3;
		TPD_INFO("edge_sliding_matched_support config using default.\n");
	} else {
		grip_info->edge_sliding_matched_support = temp_array[0];
		grip_info->edge_sliding_exit_yfsr_thd = temp_array[1];
		grip_info->edge_sliding_exit_distance = grip_info->max_y / temp_array[2];
	}
	return 0;
}
